...
import groupExpensesFakerFactory from "./groupExpensesFakerFactory";
...
const GroupExpensesFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [paidByIds, setpaidByIds] = useState();
    ...
    const onRun = async () => {
        let fakeData = groupExpensesFakerFactory(props.user,fakerCount,paidByIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("groupExpenses").create(elem));
...
export default connect(mapState, mapDispatch)(GroupExpensesFakerDialogComponent);
