...
import liabilitiesFakerFactory from "./liabilitiesFakerFactory";
...
const LiabilitiesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = liabilitiesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("liabilities").create(elem));
...
export default connect(mapState, mapDispatch)(LiabilitiesFakerDialogComponent);
