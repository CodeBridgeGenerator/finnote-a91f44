...
import groupsFakerFactory from "./groupsFakerFactory";
...
const GroupsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = groupsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("groups").create(elem));
...
export default connect(mapState, mapDispatch)(GroupsFakerDialogComponent);
