...
import assetsFakerFactory from "./assetsFakerFactory";
...
const AssetsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = assetsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("assets").create(elem));
...
export default connect(mapState, mapDispatch)(AssetsFakerDialogComponent);
