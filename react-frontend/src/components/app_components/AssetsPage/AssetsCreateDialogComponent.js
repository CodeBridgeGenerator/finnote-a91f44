...
const AssetsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,type: _entity?.type,amount: _entity?.amount,description: _entity?.description,
        ...
        const result = await client.service("assets").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Assets created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Assets" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Assets" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="assets-create-dialog-component">
...
export default connect(mapState, mapDispatch)(AssetsCreateDialogComponent);
