...
import recurringExpensesFakerFactory from "./recurringExpensesFakerFactory";
...
const RecurringExpensesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = recurringExpensesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("recurringExpenses").create(elem));
...
export default connect(mapState, mapDispatch)(RecurringExpensesFakerDialogComponent);
