...
import RecurringExpensesDatatable from "./RecurringExpensesDataTable";
import RecurringExpensesEditDialogComponent from "./RecurringExpensesEditDialogComponent";
import RecurringExpensesCreateDialogComponent from "./RecurringExpensesCreateDialogComponent";
import RecurringExpensesFakerDialogComponent from "./RecurringExpensesFakerDialogComponent";
import RecurringExpensesSeederDialogComponent from "./RecurringExpensesSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const RecurringExpensesPage = (props) => {
    ...
    const filename = "recurringExpenses.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("recurringExpenses");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "RecurringExpenses", type: "error", message: error.message || "Failed get RecurringExpenses" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("recurringExpenses").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "RecurringExpenses", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("recurringExpenses").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/recurringExpenses/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="recurringExpenses-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="recurringExpenses-datatable">
                    <RecurringExpensesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <RecurringExpensesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <RecurringExpensesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <RecurringExpensesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <RecurringExpensesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(RecurringExpensesPage);
